...
void gdt_initialize() {
    ...
    // Code segment
    gdt_set_gate(1, 0, 0xFFFFFFFF, 0x9A, 0xCF); /* DPL ring 0 */
    // Data segment
    gdt_set_gate(2, 0, 0xFFFFFFFF, 0x92, 0xCF);
    // User code segment
    gdt_set_gate(3, 0, 0xFFFFFFFF, 0xFA, 0xCF); /* DPL ring 3 */
    // User data segment
...
void page_fault_error(regs_t *r) {
    ...
    serial_dump_process(current_process);
    serial_dump_page_directory(current_process->pd);

    kpanic("Page fault! (%s%s%s%s%s) at 0x%x\nFaulting process: %d\n", (flags & 0x1) ? "Present |" : "Not present |", (flags & 0x2) ? "Write |" : "Read |", (flags & 0x4) ? "User |" : "Supervisor |", (flags & 0x8) ? "Reserved bit set |" : "", (flags & 0x10) ? "Instruction fetch" : "", faulting_address, current_process != NULL ? current_process->pid : -1);
